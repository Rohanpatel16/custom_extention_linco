...
> ${data.headline}`);
    ...
    lines.push("");
    const meta = [];
    if (data.location) meta.push(`\u{1F4CD} ${data.location}`);
    if (data.followers) meta.push(`\u{1F465} ${data.followers} followers`);
    if (data.connections) meta.push(`\u{1F517} ${data.connections} connections`);
    if (data.mutualConnections) meta.push(`\u{1F91D} ${data.mutualConnections} mutual`);
    if (meta.length) lines.push(meta.join("  \xB7  "));
    if (data.url) lines.push(`
\u{1F517} [View Profile](${data.url})`);
    lines.push("");
...
${p.description}`);
    ...
    if (data.activity && data.activity.length > 0) {
      ...
      data.activity.forEach((act) => {
        const emoji = act.isRepost ? "\u{1F501}" : "\u{1F4DD}";
...
Write the message now.`;
      document.getElementById("prompt-input").value = result.customPrompt !== void 0 ? result.customPrompt : defaultPrompt;
    });
...
Write the message now.`;
  ...
  async function generateConnectionRequest() {
    ...
    try {
      ...
      aiErrorBox.style.display = "block";
      showToast("Generation failed", "error", 3e3);
    } finally {
      isGenerating = false;
...
"[Suggested live call script. Keep it to 2-3 sentences max. State their probable challenge based on their hiring signals, introduce the value hook naturally, and close with a low-friction question.]"`;
  ...
  async function generateCallPrep() {
    ...
    try {
      ...
      prepErrorBox.style.display = "block";
      showToast("Prep generation failed", "error", 3e3);
    } finally {
  ...
  async function copyCallPrep() {
    ...
    try {
      ...
      showToast("Call Prep copied!", "success", 2e3);
      setTimeout(() => {
  ...
  async function copyConnectionRequest() {
    ...
    try {
      ...
      copyBtn.textContent = "\u2713 Copied!";
      copyBtn.classList.add("copied");
      showToast("Copied to clipboard!", "success", 2e3);
      setTimeout(() => {
        copyBtn.textContent = originalText;
        copyBtn.classList.remove("copied");
      }, 2e3);
